...
#ifdef __OFED_4_8__
static ssize_t irdma_show_attr_sq_enforce_size_enable(struct irdma_vsi_grp *grp, char *buf)
{
#else
...
#ifdef __OFED_4_8__
static ssize_t irdma_store_attr_sq_enforce_size_enable(struct irdma_vsi_grp *grp,
						       const char *buf,
						       size_t count)
#else
/**
...
#ifdef __OFED_4_8__
	ret = irdma_configfs_set_vsi_attr(grp, buf, IRDMA_ATTR_ENFORCE_SQ_SIZE);
#else
...
/* roce_rca_config_show - Show rca config value
 * @item: config item
 * @buf: buffer to write to
 */
static ssize_t roce_rca_config_show(struct config_item *item, char *buf)
{
	struct irdma_vsi_grp *grp = container_of(to_config_group(item),
						 struct irdma_vsi_grp,
						 group);
...
/**
 * roce_rca_config_store - Store rca config value
 * @item: config item
 * @buf: buf to read from
 * @count: size of buf
 */
static ssize_t roce_rca_config_store(struct config_item *item,
				     const char *buf,
				     size_t count)
{
	struct irdma_vsi_grp *grp = container_of(to_config_group(item),
...
CFG_CONFIG_DESC_ITEM_ATTR(sq_enforce_size_enable);
